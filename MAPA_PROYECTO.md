...
| Login rediseñado "SysGym Motion" (tabs cuenta individual / DNI-gimnasio, login social, demo sin registro) | `src/app/login/page.tsx`, `src/app/login/login-form.tsx` (tabs "Cuenta individual"/"Con DNI (Gimnasio)", botones Google/Facebook/X, link `/demo`), `src/components/auth/google-one-tap.tsx` | ✅ HECHO — en producción, no documentado hasta ahora |
| Consola Dev Cockpit (`/admin`, superadmin, gimnasio `sante`) | `src/app/admin/page.tsx`, `admin/vista-switcher.tsx` (entrar como Dueño/Socio 1-click sin contraseña), `admin/credenciales-card.tsx` (credenciales de testing por gimnasio), `admin/reset-claves-dev.tsx` + `admin/boton-reset-clave.tsx` (reset rápido de claves), `admin/selector-plan-rapido.tsx` (forzar plan Elite/Pro/Básico), `admin/salud/page.tsx` (diagnóstico uso Supabase), accesos directos a `admin/partner/`, `admin/gimnasios/`, `admin/errores/page.tsx`, `admin/push-prueba/`, `admin/simulador-rutina/` | ✅ HECHO — en producción, no documentado hasta ahora |
...
| Cron `recalcular_estado_cuota()` | `src/app/api/cron/cuotas/route.ts` (llama `recalcular_estado_cuota`, `chequear_trial_vencido`, `chequear_plan_vencido`), `vercel.json` (`0 12 * * *`) | ✅ HECHO — corre diario vía Vercel Cron (verificado 2026-09-15, contradecía este mapa) |
