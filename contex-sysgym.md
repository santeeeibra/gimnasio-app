...
- **UI & UX (Sistema de Experiencia SysGym)**: Rigiéndose estrictamente por el Stack de Skills en `.agents/skills/`:
  1. **`sysgym-ux-patterns`**: Feedback sensorial háptico y acústico (`src/lib/ui/hapticos.ts` + Web Audio API + iOS 17.4+ Taptic Engine + Android Vibration API), pickers drum-roll y dials de regla estilo iOS.
  2. **`apple-design-skill`**: Estilo nativo iOS, mobile-first, targets táctiles ≥44px, curvaturas squircle (`rounded-[10px]`, `rounded-[12px]`).
  3. **`60fps-animation`**: Animaciones 60fps/120fps sobre GPU (`transform` y `opacity`), física de resortes iOS, FLIP en React, soporte ProMotion 120Hz.
  4. **`sysgym-mascot-skill`**: Mascota oficial (Pulpo Volt verde `#10e7a0` con muñequera deportiva, `<Pulpo />`, `<PulpoCard />`, `<MascotaLoading />`, `<PullToRefresh />`, cero cajas blancas, cero solapamientos en UI).
  5. **`science-workout-engine`**: Motor de rutina basado estrictamente en evidencia científica (Schoenfeld, Israetel, Helms, Beardsley, RIR, DUP, sustituciones finas).
  6. **`ios-ux-prototype`**: Prototipado interactivo de flujos UX móviles.
- **UX**: todo a máximo 2 taps del panel principal. Que ni dueño ni cliente pierdan tiempo buscando.
...
| Rediseño UI mobile-first con skills de SysGym | 🔄 EN CURSO — `/login` ✅. `/panel/ajustes` ✅ (rediseño guiado hecho 2026-09-02: paletas prearmadas + "Personalizar a mano" plegado + preview sticky). `/mi/rutina` editor ✅ (miniaturas + visor + táctil). **`/mi` home + form de generar/regenerar rutina ✅ (SPEC `SPEC_UI_HOME_RUTINA.md`, 2026-09-02)**: componente `Select` reusable en `src/components/ui.tsx` (`appearance-none` + chevron SVG, `bg-paper`, `text-[16px]`) → aplicado en `generar-form.tsx` y `alta-form.tsx`; variante `volt` de `Button` (`bg-volt`/`text-volt-ink`) para "Regenerar rutina" + chips "Zona a enfocar" seleccionados en `--volt`; token `--color-volt-ink` expuesto como utility en `globals.css`; filas "Mensajes" / "Tu rutina" de `/mi` agrupadas en un `<ul>` con `divide-y` + íconos SVG inline; bottom nav de cliente `src/app/mi/mi-nav.tsx` (Inicio · Rutina · Mensajes) montada en `mi/layout.tsx`. Próximo `/panel` (dashboard). Reglas en `.agents/skills/` (`sysgym-ux-patterns`, `apple-design-skill`, `60fps-animation`, `sysgym-mascot-skill`). Dirección de tema en `INSTRUCCIONES_TEMA.md` §4 |
...
- Mobile-first + stack de skills oficiales SysGym (`.agents/skills/`) antes de tocar UI o backend.
