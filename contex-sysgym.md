...
| 3 — Push web nativo | 🔄 **Código completo (2026-09-02)** — falta solo generar VAPID keys y deploy. **SW**: `public/sw.js` (scope `/`, listeners `push` + `notificationclick`) + `public/manifest.webmanifest` (+ link/appleWebApp/themeColor en `layout.tsx`). Middleware whitelistea `/sw.js`, `/manifest.webmanifest`, `/icon-`, `/badge-`, `/api/cron`. **Suscripción**: `src/lib/push/cliente.ts` (registrar SW, `Notification.requestPermission`, `pushManager.subscribe` con VAPID). `src/app/mi/push-actions.ts` (`guardarSuscripcion` upsert on `endpoint` / `borrarSuscripcion`). Botón `src/app/mi/activar-notificaciones.tsx` (estados no-soportado/denegado/sin-suscribir/suscrito) montado en `/mi`. **Emisor**: `src/lib/push/enviar.ts` — `enviarPush(profileIds[], {title,body,url,tag})` con `web-push`, lee subs vía admin client, borra 404/410, no lanza. **Wiring**: `enviarMensaje` + `responderDueno` (avisa cliente) en `panel/mensajes/actions.ts`; `responderCliente` (avisa dueño vía admin client) en `mi/mensajes/actions.ts`. **Cron cuota**: `src/app/api/cron/cuotas/route.ts` (GET, `Authorization: Bearer $CRON_SECRET`, avisa cliente + dueño a los 6 y 1 días) + `vercel.json` cron `0 12 * * *`. Typecheck limpio ✅. `/sw.js` y `/manifest` sirven 200 sin redirect ✅; cron sin auth → 401 ✅; card "Notificaciones" renderiza en `/mi` ✅. **Pendiente manual**: `npx web-push generate-vapid-keys` → `.env.local` (`NEXT_PUBLIC_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT=mailto:…`) + `CRON_SECRET`; iconos `public/icon-192.png` / `icon-512.png` / `badge-72.png`; prueba real de suscripción + envío en navegador con permiso; deploy a Vercel (cron se activa solo). iOS: solo 16.4+ y con PWA instalada. |
...
Push web nativo (2026-09-02): código completo (SW, manifest, suscripción, emisor,
wiring en mensajería, cron de cuota + `vercel.json`). Typecheck limpio, rutas
verificadas. Falta lo manual (ver fila Entregable 3): VAPID keys + `CRON_SECRET`
en `.env.local`, iconos PNG en `public/`, prueba real en navegador, deploy.

Pendiente, prioridad sugerida:

1. **Cerrar Entregable 3**: generar VAPID keys, agregar iconos, probar
   suscripción + envío real en un navegador con permiso, deploy a Vercel.
