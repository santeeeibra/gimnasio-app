...
## Últimos Cambios
- **PoC Memoji 3D - FASE 4B, 4B.1, 4C & 4C.1 Expresividad de Boca y Cejas Memoji-Like (2026-09-16)**: Implementada expresividad de boca con `smileLeft`/`smileRight` independientes, smirk asimétrico, elevación 3D acotada con `jawOpen` coexistente limpia, y cejas independientes Memoji-like con `browInnerUp`, contracción `browDown` priorizada y perfil afinado en `src/app/poc-memoji/memoji-poc.tsx`.
- **PoC Memoji 3D - FASE 4A.1 Corrección Quirúrgica Boca (2026-09-16)**: Resueltos P0 (eliminación de z-fighting separando Y=+/-0.0024 en comisuras neutras), P1 (retraído de labios en -X para envolver la curvatura esférica de la cabeza) y P2 (incorporada cavidad bucal posterior `#020202` en `MouthAssembly` que se expande dinámicamente con `jawOpen`) en `src/app/poc-memoji/memoji-poc.tsx`.
- **PoC Memoji 3D - FASE 4A Boca Memoji-Like (2026-09-16)**: Implementada arquitectura `MouthAssembly` procedural con labio superior e inferior basados en curvas `CatmullRomCurve3` y `TubeGeometry`. Control orgánico por `jawOpen` (apertura vertical sin scaleY) y `smileLeft`/`smileRight` (curvatura de sonrisa/smirk 3D sutil) con lerp smoothing en `src/app/poc-memoji/memoji-poc.tsx`.
...
- [2026-09-16 20:21] Face rig Volt: pulpo-volt-facerig.blend tiene ojos 3D reales (Eyeball_L/R_Sclera+Pupil, bien ubicados a mano) + 18 shape keys funcionales (jawOpen OK, browUp casi no se nota, browDown ok, falta probar smile/blink/squint/browInnerUp). CUIDADO: no tocar customdata_custom_splitnormals_clear() en la cabeza, rompe el shading (cuernos + nariz hundida al mover shape keys). Usuario generando refs con GPT (sorpresa lista, guiño en curso) para subir a Tripo3D y usar como target real de shape keys. Falta: huesos Eye_L/Eye_R para mirada, exportar GLB, reconectar wiring real en memoji-poc.tsx (hoy usa cejas/ojos falsos con TubeGeometry en vez del rig real).
- [2026-09-16 20:34] Face rig Volt: agregados shape keys combinados wink (blinkRight+smileRight+squintRight) y surprised (browUpLeft/Right+browInnerUp+jawOpen) en pulpo-volt-facerig.blend, calcados de refs GUINO/SORPRENDIDO 3D.glb generadas en Tripo3D. BUG encontrado: el ojo postizo (Eyeball_L/R_Sclera+Pupil) NO se oculta cuando el parpado cierra via blink shape key, tapa el guino/blink en camara -- falta rig del ojo real (bones Eye_L/Eye_R) o esconder/encoger el mesh postizo cuando blink>0.5 antes de exportar GLB.
- [2026-09-16 20:37] Face rig Volt: creados huesos Eye_L/Eye_R (hijos de Head) en el armature, eyeballs postizos parentados a esos bones (keep_transform) para dejarlos listos para mirada. Agregados drivers scripted en scale de los 4 objetos eyeball ligados a blinkLeft/blinkRight (y wink para el derecho, porque wink es un shape key horneado independiente que no mueve blinkRight.value) -- ahora el ojo postizo se achica a 0.02 y el guino/blink se ve bien en camara sin taparse. Bug anterior resuelto. Falta: exportar GLB y reconectar wiring real en memoji-poc.tsx.
- [2026-09-16 20:42] Face rig Volt: GLB re-exportado (public/models/pulpo-volt.glb, con Eye_L/Eye_R + 20 shape keys) y reconectado el wiring real en memoji-poc.tsx: activados los morphTargetInfluences reales (se saco el '&& false'), el FaceRig procedural queda montado pero oculto (elements.group.visible=false) para no duplicar cejas/boca. Agregado gaze real rotando bones Eye_L/Eye_R con lookX/lookY, y replicada en JS la formula de ocultar eyeballs postizos en blink/wink (los drivers de Blender no exportan a glTF). Verificado en localhost:3000/poc-memoji sin camara: carga sin errores, cara real del GLB visible, sin rig falso superpuesto. Pendiente: probar con camara real (blink/smile/gaze en vivo) y triggear wink/surprised con gestos.
- [2026-09-16 21:13] Face rig Volt: fix rapido del glitch de iris (se veia/salia el ojo postizo al parpadear o hacer squint fuerte con camara real, valores reales llegan a 70%+ mucho mas alto que mis pruebas en Blender). Cambiada la formula de ocultamiento en memoji-poc.tsx (~linea 1341): antes escondia recien cerca de blink=100%, ahora arranca a esconder desde closure=0.25 (closure = max(blink, squint*0.7)) y llega a escala 0.02 en closure=1, por eye L y R. Pendiente reportado por usuario y NO resuelto todavia: cabeza se deforma tipo cuernos con shape keys fuertes (bug de custom split normals ya documentado, requiere sesion Blender aparte).
- [2026-09-17 00:37] Face rig Volt (RECOMENZANDO desde 0, plan validado con GPT): hallazgo critico en la cabeza original -- 55.506 de 67.452 vertices (82%) NO estan soldados (remove_doubles), probable causa real de los cuernos/artefactos de shading, mas fuerte que la teoria de custom split normals. Topologia del socket del ojo 100% triangulada, sin loops, descartada para reusar como parpado. Arquitectura nueva: Lid_L construido desde cero (UV sphere low-poly, 61 verts, boundary de 10 verts en un solo loop limpio, ovalizado 1.12x vertical / 0.95x horizontal con falloff en 2 anillos), pasa clearance check contra el eyeball (min 1.34% del radio, sin intersecciones). Todo esto en una copia AISLADA: public/models/pulpo-volt-facerig_lid-pilot.blend (el pulpo-volt-facerig.blend original NO se toco). Protocolo nuevo acordado tras feedback del usuario: siempre copia versionada antes de tocar geometria, cambios chicos validados antes de seguir, nunca lotes grandes. Siguiente paso: shape key 'blink' unico (no squint/wide todavia), testear en 0/0.25/0.5/0.75/1.0.
- [2026-09-17 00:50] Face rig Volt (pilot Lid_L): shape key blink v1 (78%/28%) resulto casi imperceptible visualmente pese a tener deltas reales de 10-13mm verificados por script. Se subio a v2 mas agresivo (upper 97%, lower 55%) y clearance sigue OK (min 2.49% en el punto medio) PERO visualmente el resultado en blink=1.0 se ve como geometria arrugada/rota dentro del hueco, no como un parpado cerrandose limpio -- confirmado con capturas del usuario en Blender (mis screenshots via MCP siguen cacheados/rotos esta sesion, no sirven para validar). Conclusion: rotar el boundary de un shell unico (sin separar upper/lower como piezas) no da un resultado visual limpio con solo 10 vertices de borde. Pendiente: replantear con GPT el metodo de blink antes de seguir.
- [2026-09-17 00:56] Face rig Volt (pilot Lid_L): iteracion 2 (LidUpper_L + LidLower_L separados, casquetes chicos con blink por rotacion + margen radial extra para evitar el bug de interpolacion lineal atravesando la esfera) tampoco funciono -- quedaron mal posicionados (por encima de la ceja, no tapando el hueco visible del ojo). Se cumplio el timebox de 2 iteraciones acordado con el usuario/GPT sin resultado usable. DECISION: abandonar el parpado-como-geometria-nueva por ahora. Volver al hack de escala del eyeball (ya en memoji-poc.tsx) para la demo/video, y priorizar: squint+wide sobre shape keys existentes de la cabeza, y el bug de shading tipo cuernos (candidato principal: 82% de vertices sin soldar en la cabeza, ver entrada anterior). El archivo pulpo-volt-facerig_lid-pilot.blend queda como registro del experimento fallido; pulpo-volt-facerig.blend (original) sigue intacto y es la base para seguir.
- [2026-09-17 02:14] Fix 3D Credencial Digital (Lanyard): Ajustado position camera z=20, gravity [0, -40, 0] y encuadre de la cinta y tarjeta 3D sin alterar el QR en src/components/mi/credencial-qr-modal.tsx y src/components/mi/lanyard.tsx.
- [2026-09-17 02:29] Refactor Lanyard 3D Original: Reemplazado componente por la implementación oficial exacta del autor con texture compositing (BLANK_PIXEL, FRONT/BACK UV rects, imageFit contain), cinta verde flúo #10e7a0 y QR reflejado en ambas caras en src/components/mi/lanyard.tsx y src/components/mi/credencial-qr-modal.tsx.
- [2026-09-17 02:34] Fix Notificación de Nueva Versión en Producción: Actualizada API `/api/version` (src/app/api/version/route.ts) para usar VERCEL_DEPLOYMENT_ID y VERCEL_GIT_COMMIT_SHA en Vercel Serverless (eliminado el fallback a 'prod-build' estático que bloqueaba la detección) y reducido el polling de ActualizadorApp a 25s.
